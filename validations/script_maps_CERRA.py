...
if 'climatology' in plot_types:
    ...
    print(f"climatology for range {clim_ref_start}-{clim_ref_end}")
...
for year in year_list:
    ...
    for current_plot_type in plot_types:
        ...
        if current_plot_type in ['triple_map', 'single_map', 'climatology']:
            for domain in domain_list:
                ...
                for ds in datasets_cfg:
                    ...
                    for v1,v2 in zip(v1_list,v2_list):
                        ...
                        # CLIMATOLOGY PLOT: Generate maps of climatology (average over reference period)
                        if current_plot_type == 'climatology':
                            if clim_ref_start is None or clim_ref_end is None:
                                raise ValueError("'climatology' plot type requires reference period in YML")
                            
                            # Get all files for climatology period
                            path_dict_all = load_files(root_dict, dataset_list, v1_list, model, experiment, domain)
                            
                            start_date = f"{clim_ref_start}-01-01"
                            end_date = f"{clim_ref_end}-12-31"
                            
                            # Calculate climatology for dataset 1
                            cache_key1 = f"{dataset_list[0]}_{v1}_clim"
                            if cache_key1 not in climatology_cache:
                                print(f"    Computing climatology for {dataset_list[0]} - {v1} ({clim_ref_start}-{clim_ref_end})")
                                ...
                                climatology_cache[cache_key1] = (clim1, unit)
                            else:
                                clim1, unit = climatology_cache[cache_key1]
                                print(f"    Using cached climatology for {dataset_list[0]} - {v1}")
                            
                            # TRIPLE MAP: Calculate climatology for dataset 2
                            if len(dataset_list) > 1:
                                cache_key2 = f"{dataset_list[1]}_{v2}_clim"
                                if cache_key2 not in climatology_cache:
                                    print(f"    Computing climatology for {dataset_list[1]} - {v2} ({clim_ref_start}-{clim_ref_end})")
                                    clim2, unit = calculate_climatology_for_period(
                                        path_dict_all[dataset_list[1]][v2],
                                        v2,
                                        start_date,
                                        end_date
                                    )
                                    climatology_cache[cache_key2] = (clim2, unit)
                                else:
                                    clim2, unit = climatology_cache[cache_key2]
                                    print(f"    Using cached climatology for {dataset_list[1]} - {v2}")
                                
                                # Interpolate clim2 to match clim1 grid
                                clim2_interp = clim2.interp(lat=clim1.lat, lon=clim1.lon, method="linear")
                                
                                # Generate triple map with climatologies
                                ...
                                del clim2_interp, fig
                            
                            # SINGLE MAP: Generate for each dataset
                            else:
                                for idx, dataset_name in enumerate(dataset_list):
                                    # Get appropriate climatology
                                    if idx == 0:
                                        clim_data = clim1
                                    else:
                                        cache_key = f"{dataset_name}_{v1}_clim"
                                        if cache_key in climatology_cache:
                                            clim_data, unit = climatology_cache[cache_key]
                                        else:
                                            clim_data, unit = calculate_climatology_for_period(
                                                path_dict_all[dataset_name][v1],
                                                v1,
                                                start_date,
                                                end_date
                                            )
                                            climatology_cache[cache_key] = (clim_data, unit)
                                    
                                    save_file = f'{save_dir}/{step}_{domain}_{dataset_name}_{v1}_{model}_{experiment}_climatology_{clim_ref_start}-{clim_ref_end}_single.png'
                            ...
                            # Skip the rest of processing for climatology
                            continue
                        ...
                        if current_plot_type == 'triple_map':
                            ...
                            fig = maps.triple_map(
                                ...
                                var2_name=v2,
                                units=unit,
                                dataset1=f"Dataset1-{step}",
                                dataset2=f"Dataset2-{step}",
                                diff=True,
                                vscale_name=f"{v1}_{project}"
                            )
                            plt.savefig(save_file,bbox_inches='tight')
